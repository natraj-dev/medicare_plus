...
def _header(story, styles, subtitle: str):
    ...
    header_data = [[
        ...
        Paragraph(f"<b>{subtitle}</b>", ParagraphStyle("rh",
                  parent=styles["value"], alignment=TA_RIGHT, fontSize=10)),
    ]]
    t = Table(header_data, colWidths=["60%", "40%"])
    t.setStyle(TableStyle(
        [("VALIGN", (0, 0), (-1, -1), "MIDDLE"), ("GRID", (0, 0), (-1, -1), 0, WHITE)]))
    story.append(t)
    story.append(
        Paragraph("Hospital & Appointment Management Platform", styles["sub"]))
...
def _info_table(pairs: list[tuple[str, str]], styles, cols=2) -> Table:
    ...
    for i, (k, v) in enumerate(pairs):
        cell = [Paragraph(k, styles["label"]), Paragraph(
            str(v) if v else "—", styles["value"])]
        row.append(cell)
...
def generate_prescription_pdf(prescription: Any, patient: Any, doctor: Any) -> bytes:
    ...
    story.append(_info_table([
        ...
        ("Age / Gender",
         f"{getattr(patient, 'age', '—')} / {getattr(patient, 'gender', '—')}"),
        ("Blood Group",   getattr(patient, "blood_group", "—")),
        ("Phone",         getattr(patient, "phone", "—")),
        ("Prescription #", f"RX-{prescription.id:05d}"),
        ("Date",          prescription.created_at.strftime(
            "%d %b %Y") if prescription.created_at else "—"),
        ("Valid Until",   str(prescription.valid_until)
         if prescription.valid_until else "Open"),
    ...
    try:
        meds = json.loads(prescription.medications) if isinstance(
            prescription.medications, str) else []
    except Exception:
        meds = []
    ...
    if meds:
        ...
        med_table = Table(
            rows, colWidths=[10*mm, 45*mm, 25*mm, 30*mm, 25*mm, 35*mm])
...
def generate_invoice_pdf(billing: Any, patient: Any) -> bytes:
    ...
    story.append(_info_table([
        ...
        ("Date",         billing.created_at.strftime(
            "%d %b %Y") if billing.created_at else "—"),
        ("Patient Name", getattr(patient, "full_name", "—")),
        ("Phone",        getattr(patient, "phone", "—")),
    ...
    rows = [[
        ...
        Paragraph("<b>Amount (₹)</b>",  ParagraphStyle("ra",
                  parent=s["bold"], alignment=TA_RIGHT)),
    ]]
    items = [
        ...
        if amt and amt > 0:
            rows.append([
                ...
                Paragraph(f"₹ {amt:,.2f}", ParagraphStyle(
                    "ra2", parent=s["value"], alignment=TA_RIGHT)),
            ])

    if billing.discount:
        rows.append([
            Paragraph("Discount", s["value"]),
            Paragraph(f"- ₹ {billing.discount:,.2f}", ParagraphStyle("disc",
                      parent=s["value"], textColor=SUCCESS, alignment=TA_RIGHT)),
        ])
    if billing.tax:
        rows.append([
    ...
    rows.append([
        Paragraph("<b>TOTAL AMOUNT</b>",
                  ParagraphStyle("tot", parent=s["bold"], fontSize=11)),
    ...
    rows.append([
        ...
        Paragraph(f"₹ {billing.paid_amount:,.2f}", ParagraphStyle(
            "paid", parent=s["value"], textColor=SUCCESS, alignment=TA_RIGHT)),
    ])
...
def generate_appointment_report_pdf(appointments: list, title: str = "Appointment Report") -> bytes:
    ...
    story.append(Paragraph(
        f"Generated on {datetime.now().strftime('%d %b %Y %H:%M')}  •  Total: {len(appointments)} records", s["sub"]))
    ...
    for i, a in enumerate(appointments, 1):
        ...
        rows.append([
            ...
            Paragraph(str(a.status), ParagraphStyle(
                "sc", parent=s["small"], textColor=sc, fontName="Helvetica-Bold")),
            Paragraph((a.reason or "")[:40], s["small"]),
        ])

    tbl = Table(rows, colWidths=[8*mm, 22*mm, 14 *
                mm, 20*mm, 20*mm, 20*mm, 22*mm, 44*mm])
...
def generate_medical_summary_pdf(patient: Any, appointments: list, records: list, prescriptions: list) -> bytes:
    ...
    if appointments:
        story.append(Spacer(1, 8))
        story.append(
            Paragraph(f"Appointment History ({len(appointments)} records)", s["h2"]))
        rows = [[Paragraph(h, s["bold"])
                 for h in ["Date", "Time", "Type", "Status", "Reason"]]]
        for a in appointments[:20]:
            rows.append([
                Paragraph(str(a.appointment_date), s["value"]),
                Paragraph(str(a.appointment_time)[:5], s["value"]),
                Paragraph(str(a.appointment_type).replace(
                    "_", " "), s["small"]),
        ...
        at.setStyle(TableStyle([
            ("BACKGROUND", (0, 0), (-1, 0),
             PRIMARY), ("TEXTCOLOR", (0, 0), (-1, 0), WHITE),
            ("ROWBACKGROUNDS", (0, 1), (-1, -1), [WHITE, PRIMARY_L]),
    ...
    if records:
        story.append(Spacer(1, 8))
        story.append(
            Paragraph(f"Medical Records ({len(records)} files)", s["h2"]))
        rows = [[Paragraph(h, s["bold"])
                 for h in ["Type", "Title", "Date", "Doctor", "Hospital"]]]
        for r in records[:15]:
            rows.append([
